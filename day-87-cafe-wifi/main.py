...
class CafeFrom(FlaskForm):
    ...
    has_toilet = SelectField('Has Toilet', choices=[(1, '✔'), (0, '✘')], validators=[DataRequired()])
    has_wifi = SelectField('Has Wifi', choices=[(1, '✔'), (0, '✘')], validators=[DataRequired()])
    has_sockets = SelectField('Has Sockets', choices=[(1, '✔'), (0, '✘')], validators=[DataRequired()])
    can_take_calls = SelectField('Can Take Calls', choices=[(1, '✔'), (0, '✘')], validators=[DataRequired()])
...
def add():
    ...
    if request.method == 'POST' and form.validate_on_submit():
        ...
        if Cafe.query.filter_by(name=new_cafe.name).first():
            flash(f"{new_cafe.name} already exists in 'Cafe and Wifi, please try again!'")
